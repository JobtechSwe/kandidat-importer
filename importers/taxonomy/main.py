...
def create_valuestore_jobs(taxonomy_jobterms, taxonomy_jobgroups, taxonomy_jobfields):
    jobfields = {field['LocaleFieldID']: OrderedDict([('id',str(field['LocaleFieldID'])),\
                                                      ('type', settings.taxonomy_type['yrkesomrade']),\
                                                      ('label', field['Term']), ('description', field['Description'])]) \
                for field in taxonomy_jobfields}
    jobgroups = {field['LocaleCode']: OrderedDict([('id', str(field['LocaleCode'])), \
                                                   ('type', settings.taxonomy_type['yrkesgrupp']),\
                                                   ('label', field['Term']), ('description', field['Description']), \
                                                   ('parent', jobfields[field['LocaleFieldID']])]) \
                for field in taxonomy_jobgroups}
    jobterms = {field['OccupationNameID']: OrderedDict([('id', str(field['OccupationNameID'])), \
                                                        ('type', settings.taxonomy_type['yrke']), \
                                                        ('label', field['Term']), ('parent', jobgroups[field['LocaleCode']])])\
...
def create_valuestore_geo(taxonomy_municipalities, taxonomy_regions):
    regions = {field['NationalNUTSLevel3Code']: OrderedDict([('id', str(field['NationalNUTSLevel3Code'])), \
                                                             ('type', settings.taxonomy_type['lanskod']), \
                                                             ('label', field['Term']), \
                                                             ('num_id', int(field['NationalNUTSLevel3Code']))]) \
               for field in taxonomy_regions}
    municipalities = {field['NationalNUTSLAU2Code']: OrderedDict([('id', str(field['NationalNUTSLAU2Code'])), \
                                                                  ('type', settings.taxonomy_type['kommunkod']), \
                                                                  ('label', field['Term']), \
...
def create_valuestore_skills(taxonomy_skills):
    skills = {field['SkillID']: OrderedDict([('id', str(field['SkillID'])), \
...
def create_valuestore_work_time_extent(taxonomy_work_time_extent):
    wte = {field['WorkTimeExtentID']: OrderedDict([('id', str(field['WorkTimeExtentID'])), \
...
def create_valuestore_languages(taxonomy_languages):
    languages = {field['LanguageID']: OrderedDict([('id', str(field['LanguageID'])), \
                                                   ('type', settings.taxonomy_type['sprak']),\
                                                   ('label',field['Term']), ('num_id', int(field['LanguageID']))]) \
...
def fetch_full_taxonomy():
    try:
        ...
        raise
    (valuestore_jobterm, valuestore_jobgroup, valuestore_jobfield) = create_valuestore_jobs(taxonomy_jobterms, taxonomy_jobgroups, taxonomy_jobfields)
    (valuestore_municipalities, valuestore_regions) = create_valuestore_geo(taxonomy_municipalities,taxonomy_regions)
    valuestore_languages = create_valuestore_languages(taxonomy_languages)
    valuestore_work_time_extent = create_valuestore_work_time_extent(taxonomy_work_time_extent)
    valuestore_skills = create_valuestore_skills(taxonomy_skills)
    return(list(valuestore_jobterm.values()) + list(valuestore_jobgroup.values()) +\
           list(valuestore_jobfield.values()) +\
           list(valuestore_municipalities.values()) +\
           list(valuestore_regions.values()) + list(valuestore_languages.values()) +\
...
def check_if_taxonomyversion_already_exists():
    ...
    log.info('Expected index name based on taxonomy service version is {}'.format(expected_index_name))
    try:
        ...
        try:
            if (elastic.alias_exists(settings.ES_TAX_INDEX_ALIAS)):
                ...
                if (elastic.alias_exists(settings.ES_TAX_ARCHIVE_ALIAS)):
                    elastic.add_indices_to_alias(list(alias.keys(),settings.ES_TAX_ARCHIVE_ALIAS))
                else:
                    elastic.put_alias(list(alias.keys()),settings.ES_TAX_ARCHIVE_ALIAS)
                    elastic.update_alias(indexname,list(alias.keys()), settings.ES_TAX_INDEX_ALIAS)
            else:
