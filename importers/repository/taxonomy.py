...
def get_term(taxtype, taxid):
    ...
    taxonomy_entity = find_concept_by_legacy_ams_taxonomy_id(elastic.es,
                                                             taxtype, taxid, {})
    label = None
...
def get_entity(taxtype, taxid, not_found_response=None):
    ...
    taxonomy_entity = elastic.es.get_source(index=ES_TAX_INDEX,
                                            id=doc_id,
...
def find_concept_by_legacy_ams_taxonomy_id(taxonomy_type,
    ...
    try:
        elastic_response = elastic.es.search(index=ES_TAX_INDEX, body=query)
...
def find_concepts(query_string=None, taxonomy_code=[], entity_type=None,
                  offset=0, limit=10):
    query_dsl = _build_query(query_string, taxonomy_code, entity_type, offset, limit)
    log.debug("Query: %s" % json.dumps(query_dsl))
    try:
        elastic_response = elastic.es.search(index=ES_TAX_INDEX, body=query_dsl)
